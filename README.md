...
- **Серт Минцифры** — зашитый корень Russian Trusted Root/Sub CA под флагом, чтобы
  ходить на `api2.oneme.ru` (пока единственный такой хост) без установки серта в систему.
...
Пока это нужно только для **`api2.oneme.ru`**: он отдаёт серт под корнем
**Russian Trusted Root/Sub CA (Минцифры)**, которого нет в бандле Mozilla, и без
него TLS падает с `UnknownIssuer`, если пользователь не поставил корень в систему.
Остальные хосты (основной `api.oneme.ru`, CDN, ws2) пока под обычными публичными
сертами. Флаг при этом покрывает все три пути TLS сразу — если Макс переведёт на
Минцифру ещё что-то, менять код не придётся.
...
— по добавленному корню. Три пути TLS — это основной сокет, медиа-CDN и ws2-звонки;
флаг процессный, ставь один раз на старте, до сессий/загрузок/звонков.
