"""Answering music bot (asyncio-native): logs in, waits for an incoming call,
auto-accepts, and plays an audio track into it.

Same flow as call_bot.py, but built on kolibri's AsyncSession / AsyncCallSignaling
instead of wrapping the blocking API in asyncio.to_thread. Signaling goes through
kolibri (main socket + ws2); the WebRTC media is aiortc. This is a P2P (1:1)
callee - it answers the caller's offer and streams the track.

    KOLIBRI_TRACK=song.opus KOLIBRI_PHONE=+7... python examples/call_bot_async.py
...
async def login(session, device_id: str):
    ...
    if not token:
        phone = os.environ.get("KOLIBRI_PHONE") or (
            await asyncio.to_thread(input, "bot phone (+7...): ")
        ).strip()
        mode = kolibri.auth_mode(seed, device_id)
        r = await session.request(AUTH_REQUEST, {"phone": phone, "type": "START_AUTH",
                                                 "language": "ru", "mode": mode})
        code = (await asyncio.to_thread(input, "SMS code: ")).strip()
        v = await session.request(AUTH, {"token": r["token"], "verifyCode": code,
...
async def handle_call(sig, params):
    ...
    await sig.accept_call()
    await sig.change_media_settings(has_audio, has_video, False)
    log("accepted; playing", os.path.basename(TRACK))

    while True:
        n = await sig.next_notification(60.0)
        if n is None:
        ...
        if name == "connection":
            ...
            if td["kind"] == "sdp" and td["type"] == "offer":
                ...
                await sig.transmit_sdp(peer_id, "answer", pc.localDescription.sdp)
                for cand, mid, mline in local_candidates(pc.localDescription.sdp):
                    await sig.transmit_candidate(peer_id, cand, mid, mline)
                log("answered")
...
async def main():
    ...
    await login(session, device_id)

    log("waiting for an incoming call... (call this account now)")
    while True:
        push = await session.next_push(3600.0)
        ...
        if params is None:
            ...
            continue
        sig = await kolibri.AsyncCallSignaling.connect(params["ws2_url"])
