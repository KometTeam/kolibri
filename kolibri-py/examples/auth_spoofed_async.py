"""Spoofed-device handshake + START_AUTH over asyncio (sends a REAL SMS).

    python examples/auth_spoofed_async.py +7XXXXXXXXXX
"""
...
async def main() -> None:
    if len(sys.argv) < 2:
        sys.exit("usage: python examples/auth_spoofed_async.py +7XXXXXXXXXX")
    ...
    resp = await s.request(AUTH_REQUEST, {
        "phone": phone,
        "type": "START_AUTH",
        "language": "ru",
        "mode": mode,
