//! C ABI over kolibri-net, for the Swift binding. Blocking calls over an owned
//! tokio runtime, like the Python/Dart wrappers; bytes cross the boundary as
//! `(ptr, len)`. Fallible calls return an owned error string (NULL on success)
//! and write results into out-params.
...
// SAFETY: the Swift side owns `user` and its callback is safe to call from any
